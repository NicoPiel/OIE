...
function getRowPrefixes(
  row: Row<DashboardRowModel>,
  inverseDepth = 0
): Array<ReactNode> {
  if (!row.getParentRow()) {
    return [];
  ...
  return [
    ...
    <div
      key={inverseDepth}
      className={css.treePrefixSymbol}
      data-symbol-type={thisPrefix}
    ></div>,
  ];
}

function ChannelNameCell({ row }: { row: Row<DashboardRowModel> }) {
  const prefix = getRowPrefixes(row);
  const name = row.original.name;
  const content =
    'channelId' in row.original ? (
      <Link to={`/channels/${row.original.channelId}`}>{name}</Link>
    ) : (
      name
    );

  return (
    <div className={css.channelNameCell}>
      <div className={css.treePrefix}>
        {prefix}
        {row.getCanExpand() && (
          <button
            type='button'
            className={`${css.expanderButton} ${
              row.getIsExpanded() ? css.expanded : ''
            }`}
        ...
        {row.getCanExpand() && (
          <img
            ...
            src={!row.getParentRow() ? serverDatabaseIcon : serverIcon}
            alt='icon'
          />
        )}
      </div>
      <div className={css.channelNameText}>{content}</div>
    </div>
  );
}
...
function StatusCell({ row }: { row: Row<DashboardRowModel> }) {
  return (
    <>
      <span
        style={{ fontSize: '5pt', verticalAlign: '3px', paddingRight: '2px' }}
      >
        {iconForStatus(row.original.state)}
      </span>{' '}
      {row.original.state}
    </>
  );
...
function RevisionCountCell({ row }: { row: Row<DashboardRowModel> }) {
  ...
  return (
    <>
      {row.original.deployedRevisionDelta > 0
        ? `+${row.original.deployedRevisionDelta}`
        : '--'}
    </>
  );
}
...
const DashboardColumns: ColumnDef<DashboardRowModel>[] = [
  ...
  {
    header: 'Rev Δ',
    accessorKey: 'deployedRevisionDelta',
    cell: RevisionCountCell,
  },
  {
...
export function DashboardList() {
  ...
  const [prefs, setPrefs] = usePreferences<FilterPreferences>(
    DASHBOARD_PREFS_KEY,
    () => ({
      textFilter: '',
      useGroups: true,
  ...
  const refreshIntervalSeconds =
    typeof sess.prefs.intervalTime === 'number' ? sess.prefs.intervalTime : 10;
  ...
  const rows = useMemo<DashboardRowModel[]>(
    () => dashboard?.groups ?? [],
    [dashboard]
  );
  ...
  return (
    <div className={`card p-2 ${css.dashboardListCard}`}>
      ...
      <div className={css.toolbar}>
        ...
        {dashboard && (
          <div className={css.dashboardSummary}>
            {dashboard.groups.length.toLocaleString()} Groups,{' '}
            {dashboard.deployedChannelCount.toLocaleString()} Deployed Channels
          </div>
        )}
        ...
        <div className={css.statsModeSelector}>
          <input
            type='radio'
            id='dashboard-stats-mode'
            value='Current'
            name='dashboard-stats-mode'
            checked={prefs.statsMode === 'Current'}
            onChange={() => setPrefs({ statsMode: 'Current' })}
          />
          <label htmlFor='dashboard-stats-mode' className='ms-1 me-2'>
            Current Statistics
          </label>

          <input
            type='radio'
            id='dashboard-stats-mode-lifetime'
            value='Lifetime'
            name='dashboard-stats-mode'
            checked={prefs.statsMode === 'Lifetime'}
            onChange={() => setPrefs({ statsMode: 'Lifetime' })}
          />
          <label htmlFor='dashboard-stats-mode-lifetime' className='ms-1 me-2'>
            Lifetime Statistics
          </label>
        </div>
        ...
        <button
          className={`btn btn-outline ${
            prefs.tagDisplayMode === 'Names' ? 'active' : ''
          }`}
          onClick={() => toggleTagDisplayMode('Names')}
          title='Toggle Tag Names'
          aria-label='Toggle Tag Names'
        >
          <img src={styleIcon} aria-hidden='true' />
        </button>
        <button
          className={`btn btn-outline ${
            prefs.tagDisplayMode === 'Icons' ? 'active' : ''
          }`}
          onClick={() => toggleTagDisplayMode('Icons')}
          title='Toggle Tag Icons'
          aria-label='Toggle Tag Icons'
        >
        ...
        <button
          className={`btn btn-outline ${prefs.useGroups ? 'active' : ''}`}
          onClick={() => setPrefs({ useGroups: true })}
          title='Show Groups'
        ...
        <button
          className={`btn btn-outline ${!prefs.useGroups ? 'active' : ''}`}
          onClick={() => setPrefs({ useGroups: false })}
          title='Hide Groups'
